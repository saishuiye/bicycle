...
def forget():
    ...
    if user:
        ...
        logic.send_email(email)
    else:
        context['msg'] = '此邮箱从未注册过！'
...
def post():
    ...
    url = 'http://127.0.0.1:5001/lease_frequency/'
...
def upload():
    ...
    if request.method == 'POST':
        f = request.files['file']
        uuid_filename = str(uuid.uuid1()) + "_" + f.filename;
        basepath = os.path.dirname(__file__)  # 当前文件所在路径
        upload_path = os.path.join(basepath, 'static\uploads',secure_filename(uuid_filename))  #注意：没有的文件夹一定要先创建，不然会提示没有该路径
        f.save(upload_path)
        file = File(uuidname=uuid_filename, realname=f.filename, savepath=upload_path, description=description )
        db.session.add(file)
        db.session.commit()
        context['is_success'] = True
